...
export default function StandardsView() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-8 space-y-8">
        ...
        {standards.map((std) => {
          ...
          return (
            ...
            >
              ...
              <div className="text-xs text-slate-500 mt-1 line-clamp-1">{std.name}</div>
              <div className="mt-3 flex items-center justify-between text-[11px]">
                <span className="text-blue-700 font-mono font-semibold">{std.ruleCount} Rules</span>
                <span className="text-slate-500">{std.category}</span>
              </div>
            </div>
          );
        })}
      ...
      {/* Rules Table */}
      <div className="pro-card overflow-hidden space-y-4 p-6">
              ...
              <div key={rule.id} className="py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    <span className="font-mono text-xs font-bold text-slate-900">{rule.id}</span>
                    <span
                    ...
                    <span className="text-[11px] font-mono text-blue-800 bg-blue-50 px-2 py-0.5 rounded border border-blue-200 font-semibold">
                      {rule.clause}
                    </span>
                  </div>
                  <h4 className="text-xs font-semibold text-slate-900">{rule.name}</h4>
                  <p className="text-xs text-slate-500">{rule.description}</p>
                </div>

                <div className="flex items-center space-x-4 shrink-0">
                  <div className="text-right text-[11px]">
                    <div className="text-slate-600 font-mono flex items-center space-x-1">
                      <Sliders className="h-3 w-3 text-blue-600" />
                      <span>{rule.parameter}</span>
                    </div>
                    <span className="text-emerald-700 font-semibold flex items-center justify-end space-x-1 mt-0.5">
